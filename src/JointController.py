...
class JointCommands_msg_handler(object):
    ...
    def __init__(self,robot_name,robot_joints):
        ...
        self._compub = rospy.Publisher('/'+self.RobotName+'/atlas_command', AtlasCommand)
    ...
    def set_default_gains_from_param(self,param_name = 0):
        if param_name == 0:
            param_name = '/'+self.RobotName+'_controller'
    ...
    def set_all_pos(self,pos_vec):
        if len(pos_vec) == len(self.JointNames):
            ...
            print 'position command length doesn\'t fit'
...
class hand_joint_controller(JointCommands_msg_handler):
    def __init__(self,hand):
        if hand == 'left':
            jnt = ['left_f0_j0', 'left_f0_j1', 'left_f0_j2', 'left_f1_j0', 'left_f1_j1', 'left_f1_j2', 'left_f2_j0', 'left_f2_j1', 'left_f2_j2', 'left_f3_j0', 'left_f3_j1', 'left_f3_j2']
            hnd = 'l'
        else:
            if hand == 'right':
                jnt = ['right_f0_j0', 'right_f0_j1', 'right_f0_j2', 'right_f1_j0', 'right_f1_j1', 'right_f1_j2', 'right_f2_j0', 'right_f2_j1', 'right_f2_j2', 'right_f3_j0', 'right_f3_j1', 'right_f3_j2']
                hnd = 'r'
            else:
                raise ValueError
        self._hand = hand
        JointCommands_msg_handler.__init__(self,'sandia_hands/'+hnd+'_hand',jnt)
        self._compub.unregister()
        self._compub = rospy.Publisher('/'+self.RobotName+'/joint_commands', JointCommands)
        self._command = JointCommands()
        
    def set_default_gains_from_param(self,param_name = '/sandia_hands'):
        JointCommands_msg_handler.set_default_gains_from_param(self,param_name)
